...
describe('utils/parse', () => {
  ...
  describe('parse.queryStringToObject()', () => {
    const defaultExpectations: Record<string, Record<string, string>> = {
      '?page=1': { page: '1' },
      '?orderBy=name': { orderBy: 'name' },
      '?page=1&orderBy=name': { page: '1', orderBy: 'name' },
    };

    const differentCasesExpectations: Record<string, Record<string, string>> = {
      '?order-by=name': { orderBy: 'name' },
      '?page=1&order-by=name': { page: '1', orderBy: 'name' },
      '?you-Come-from=google.com': { youComeFrom: 'google.com' },
    };
    ...
    Object.keys(defaultExpectations).forEach((source) => {
      ...
      it(`should parse "${source}" to ${expected}`, () => {
        expect(parse.queryStringToObject(source)).toEqual(expected);
      });
    });

    Object.keys(differentCasesExpectations).forEach((source) => {
      const expected = differentCasesExpectations[source];

      it(`should normalise case and parse "${source}" to ${expected}`, () => {
        expect(parse.queryStringToObject(source)).toEqual(expected);
      });
    });
  });
